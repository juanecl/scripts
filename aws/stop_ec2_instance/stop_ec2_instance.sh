...
# -----------------------------------------------------------------------------
# Script: stop_instance.sh
# Description: This script stops an AWS EC2 instance and verifies its status.
...
#              to stop. If the instance does not stop within 10 minutes, it
...
MESSAGE="$(date) INFO: Stopping AWS EC2 instance $TAG ($INSTANCE_ID)"
echo $MESSAGE

/usr/bin/aws ec2 stop-instances --instance-ids $INSTANCE_ID --region $REGION > /dev/null 2>&1

# Wait up to 10 minutes for the instance to stop
for i in {1..60}; do
    ...
    if [ "$INSTANCE_STATE" == "stopped" ]; then
        echo "$(date) INFO: EC2 instance $INSTANCE_ID ($TAG) has stopped successfully"
        echo $MESSAGE | mail -s "$(date) INFO: EC2 instance $INSTANCE_ID ($TAG) stopped" <put_your_email_here>
        exit 0
    fi
    sleep 10
done

# If the instance has not stopped after 10 minutes, send an email notification
echo "$(date) INFO: EC2 instance $INSTANCE_ID ($TAG) did not stop after 10 minutes" | mail -s "Error stopping EC2 instance $INSTANCE_ID ($TAG)" <put_your_email_here>